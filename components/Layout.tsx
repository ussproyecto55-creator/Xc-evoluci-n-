...
import { Home, TrendingUp, Users, User, ArrowLeftRight, Settings2, Crown, Shield, Send, ClipboardList } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-900 pb-20">
      ...
      <main className="flex-1 w-full max-w-md mx-auto flex flex-col">
        ...
        <footer className="mt-12 px-6 py-10 border-t border-white/5 bg-slate-950/30 text-center space-y-6">
          ...
          <div className="flex flex-col items-center space-y-4">
            <a href="https://t.me/+xb9nTH7qSvs0ZjFh" target="_blank" className="flex items-center space-x-2 px-6 py-2 bg-blue-500/10 border border-blue-500/20 rounded-full text-blue-400 text-[10px] font-black uppercase tracking-widest hover:bg-blue-500/20 transition-all">
              <Send size={14} />
            ...
            <p className="text-[10px] text-slate-600 font-medium leading-relaxed max-w-[280px] mx-auto italic">
              Este sitio web es exclusivamente para fines <span className="text-slate-400 font-bold">informativos y de simulación deportiva</span>. Elite Sports no es una entidad bancaria.
            </p>
          </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 glass border-t border-white/5 px-2 py-3 flex justify-between items-center z-50 max-w-md mx-auto">
        <button onClick={() => setActiveTab('home')} className={`flex flex-col items-center space-y-1 transition-all flex-1 ${activeTab === 'home' ? 'text-amber-500 scale-110' : 'text-slate-400'}`}>
          <Home size={20} />
          <span className="text-[8px] font-medium">Inicio</span>
        </button>
        <button onClick={() => setActiveTab('vip')} className={`flex flex-col items-center space-y-1 transition-all flex-1 ${activeTab === 'vip' ? 'text-amber-500 scale-110' : 'text-slate-400'}`}>
          <Crown size={20} />
          <span className="text-[8px] font-medium">VIP</span>
        </button>
        <button onClick={() => setActiveTab('bet')} className={`flex flex-col items-center space-y-1 transition-all flex-1 ${activeTab === 'bet' ? 'text-amber-500 scale-110' : 'text-slate-400'}`}>
          <TrendingUp size={20} />
          <span className="text-[8px] font-medium">Invertir</span>
        </button>
        <button onClick={() => setActiveTab('plays')} className={`flex flex-col items-center space-y-1 transition-all flex-1 ${activeTab === 'plays' ? 'text-amber-500 scale-110' : 'text-slate-400'}`}>
          <ClipboardList size={20} />
          <span className="text-[8px] font-medium">Jugadas</span>
        </button>
        <button onClick={() => setActiveTab('team')} className={`flex flex-col items-center space-y-1 transition-all flex-1 ${activeTab === 'team' ? 'text-amber-500 scale-110' : 'text-slate-400'}`}>
          <Users size={20} />
          <span className="text-[8px] font-medium">Equipo</span>
        </button>
        {isAdmin && (
          <button onClick={() => setActiveTab('admin')} className={`flex flex-col items-center space-y-1 transition-all flex-1 ${activeTab === 'admin' ? 'text-amber-500 scale-110' : 'text-slate-400'}`}>
            <Settings2 size={20} />
            <span className="text-[8px] font-medium">Admin</span>
          </button>
        )}
        <button onClick={() => setActiveTab('profile')} className={`flex flex-col items-center space-y-1 transition-all flex-1 ${activeTab === 'profile' ? 'text-amber-500 scale-110' : 'text-slate-400'}`}>
          <User size={20} />
