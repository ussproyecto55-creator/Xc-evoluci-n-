...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const dailySports = useMemo(() => {
    ...
    return SPORT_TEMPLATES.map((tpl, idx) => {
      // Deporte con ID 2 (MLB) siempre da 2.5%
  ...
  const showNotification = (message: string | undefined | null, type: NotificationType = 'info') => {
    const safeMessage = message && message.trim() !== '' ? message : 'Operación Confirmada';
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
      ...
      if (now >= endTime) {
        ...
        showNotification(`Ciclo completado: +$${totalToReturn.toFixed(2)} USDT liberados.`, "success");
      }
    }, 5000);
  ...
  const login = async (username: string, password?: string, isRegisterMode?: boolean, referredBy?: string) => {
    ...
    if (isRegisterMode) {
      ...
      if (existingUser) return { success: false, message: "Usuario ya registrado." };
      const newUser: User = {
      ...
      if (found.isBlocked) return { success: false, message: "Acceso denegado temporalmente." };
      setUser(found);
      return { success: true, message: "Bienvenido a Elite Sports." };
    }
  };
  ...
  const recharge = async (amount: number, proofData?: string) => {
    if (!user) return;
    const tx = { id: crypto.randomUUID(), userId: user.id, username: user.username, type: 'recharge', amount, status: 'pending', date: new Date().toISOString(), description: 'Carga de Liquidez USDT', proofData };
    await supabase.from('transactions').insert([tx]);
    showNotification("Depósito enviado a auditoría.", "success");
  };
  ...
  const withdraw = async (amount: number) => {
    ...
    const tx = { id: crypto.randomUUID(), userId: user.id, username: user.username, type: 'withdraw', amount, status: 'pending', date: new Date().toISOString(), description: 'Retiro de Capital', walletAddress: user.withdrawalAddress };
    await supabase.from('transactions').insert([tx]);
    await supabase.from('users').update({ balance: newBalance, monthlyWithdrawalCount: (user.monthlyWithdrawalCount || 0) + 1 }).eq('id', user.id);
    return { success: true, message: "Retiro enviado. Proceso: 24h." };
  };
  ...
  const applyCompoundInterest = async (amount: number, percent: number, sportId: string, market: string) => {
    ...
    // El ciclo dura 40 minutos para la simulación
    ...
    await supabase.from('transactions').insert([{ id: crypto.randomUUID(), userId: user.id, username: user.username, type: 'bet', amount, status: 'completed', date: new Date().toISOString(), description: `Inversión: ${market}` }]);
  };
